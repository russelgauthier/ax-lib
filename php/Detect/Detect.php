...
class Detect {
    ...
    function __construct(){
        $this->uaInfo = $this->getUserAgent();
    }

    public function getBrowser(){
        return $this->uaInfo === NULL ? NULL : $this->uaInfo["browser"];
    }
    public function getPlatform(){
        return $this->uaInfo === NULL ? NULL : $this->uaInfo["platform"];
    }
    public function getVersion(){
        return $this->uaInfo === NULL ? NULL : $this->uaInfo["version"];
    }
    private function getUserAgent($u_agent = null){
        if(is_null($u_agent)){
            if(isset($_SERVER['HTTP_USER_AGENT'])){
                $u_agent = $_SERVER['HTTP_USER_AGENT'];
            } else {
                //throw new DetectException("A user agent is required", 0x01);
            }
        }
